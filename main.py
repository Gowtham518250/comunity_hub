...
async def websocket_analytics_endpoint(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            analytics_data = {
                'heatmap': analytics_db.get_analytics()['hotspots'],
                'categories': analytics_db.get_analytics()['categories'],
                'trend': {
                    'labels': list(analytics_db.get_analytics()['trends'].keys()),
                    'values': [sum(v.values()) for v in analytics_db.get_analytics()['trends'].values()]
                }
            }
            
            # Send back real-time analytics
...
def get_complaint_analytics():
    ...
    return {
        ...
        "performance_score": calculate_performance_score(resolution_rate, avg_resolution_time, urgent_complaints)
    }

def calculate_performance_score(resolution_rate: float, avg_resolution: float, urgent_count: int) -> int:
    """Calculate overall performance score (0-100)"""
    rate_score = min(100, resolution_rate * 1.2)
    time_score = max(0, 100 - (avg_resolution * 5))
    urgent_score = min(100, (urgent_count > 0) * 80 + 20)
    return int((rate_score * 0.5) + (time_score * 0.3) + (urgent_score * 0.2))
...
async def generate_analytics_data(poll_interval: int = 5):
    ...
    while True:
        try:
            ...
            # Broadcast to all connected clients (as JSON string)
            try:
                await manager.broadcast(json.dumps({'type': 'analytics_update', 'data': payload}))
            except Exception:
...
async def startup_event():
    ...
    if admin_username not in users_db:
        ...
        save_data()
        logger.info("[Admin] Admin user created")
    
    generate_sample_complaints()
    save_data()
    logger.info("[Data] Sample complaints generated")
    
    demo_user = "demo"
    if demo_user not in users_db:
        users_db[demo_user] = {
            'username': demo_user,
            'email': 'demo@communityhub.ai',
            'phone': '+91-9876543211',
